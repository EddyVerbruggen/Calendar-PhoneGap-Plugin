...
public abstract class AbstractCalendarAccessor {
  ...
  private Event[] fetchEventInstances(String eventId, String title, String location, String notes, long startFrom,
      long startTo) {
    String[] projection = {
    ...
    Cursor cursor = queryEventInstances(startFrom, startTo, projection, selection, selectionList.toArray(selectionArgs),
        sortOrder);
    if (cursor == null) {
      return null;
    }
    ...
    // if we don't find the event by id, try again by title etc - inline with iOS
    // logic
  ...
  public final JSONArray getActiveCalendars() throws JSONException {
    ...
    if (cursor.moveToFirst()) {
      do {
        ...
        calendar.put("displayname",
            cursor.getString(cursor.getColumnIndex(this.getKey(KeyIndex.CALENDARS_DISPLAY_NAME))));
  ...
  public JSONArray findEvents(String eventId, String title, String location, String notes, long startFrom, long startTo,
    ...
    Map<String, ArrayList<Attendee>> attendeeMap = fetchAttendeesForEventsAsMap(
        eventMap.keySet().toArray(new String[0]));
  ...
  public boolean deleteEventById(Uri eventsUri, long id, long fromTime) {
    ...
    long targDtStart = -1;
    {
      ...
      // Not using a wider range because it can corrupt the Calendar Storage state!
    ...
    evRRule = TextUtils.join(";", recurRuleParts) + ";UNTIL="
        + nl.xservices.plugins.Calendar.formatICalDateTime(new Date(fromTime - 1000));
  ...
  public String createEvent(Uri eventsUri, String title, long startTime, long endTime, String description,
    ...
    if (allDayEvent) {
      // all day events must be in UTC time zone per Android specification, getOffset
    ...
    if (recurrence != null) {
      String rrule = "FREQ=" + recurrence.toUpperCase() +
          ...
          ((recurrenceEndTime > -1)
              ? ";UNTIL=" + nl.xservices.plugins.Calendar.formatICalDateTime(new Date(recurrenceEndTime))
              : "")
          +
  ...
  public String createCalendar(String accountName, String calendarName, String calendarColor) {
    try {
      ...
      Cursor result = contentResolver.query(evuri, new String[] { CalendarContract.Calendars._ID,
          CalendarContract.Calendars.NAME, CalendarContract.Calendars.CALENDAR_DISPLAY_NAME }, null, null, null);
      if (result != null) {
  ...
  public void deleteCalendar(String calendarName) {
    try {
      ...
      // also delete previously crashing calendars, see
      // https://github.com/EddyVerbruggen/Calendar-PhoneGap-Plugin/issues/241
  ...
  private void deleteCrashingCalendars(ContentResolver contentResolver) {
    ...
    if (count > 0) {
      Uri evuri = CalendarContract.Calendars.CONTENT_URI;
      Cursor result = contentResolver.query(evuri,
          new String[] { CalendarContract.Calendars._ID, CalendarContract.Calendars.ACCOUNT_NAME }, null, null, null);
      if (result != null) {
        while (result.moveToNext()) {
