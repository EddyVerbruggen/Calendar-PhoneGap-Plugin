...
public abstract class AbstractCalendarAccessor {
  ...
  protected static class Event {
    ...
    public JSONObject toJSONObject() {
      ...
      try {
		...
		//obj.putOpt("calendarName",this.calendarName);
		obj.putOpt("calendar",this.calendarName);
		obj.putOpt("calendarId",this.calendarId);
		obj.putOpt("eventId",this.eventId);
  ...
  public JSONArray findAllEventsInNamedCalendar(String calendarName) {
	String calid=null;
	Cursor cursor = queryCalendars(
                new String[]{
                        this.getKey(KeyIndex.CALENDARS_ID),
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                if(cursor.getString(cursor.getColumnIndex(this.getKey(KeyIndex.CALENDARS_NAME))).equals(calendarName))
                {
                    calid=cursor.getString(cursor.getColumnIndex(this.getKey(KeyIndex.CALENDARS_ID)));
                    break;
                }
		...
		GregorianCalendar calb = new GregorianCalendar(TimeZone.getDefault());
		calb.set(Calendar.DAY_OF_MONTH,1);
		calb.set(Calendar.MONTH,0);
		calb.set(Calendar.YEAR,1950);
        ...
        calb.set(Calendar.YEAR,2099);
        long end=calb.getTimeInMillis();
        Event[] instances = fetchEventInstances(null, null, null, null, start, end);
        if (instances == null) {
            return result;
        }
		...
		{
        	if (event.calendarId.equals(calid))
			{        	
