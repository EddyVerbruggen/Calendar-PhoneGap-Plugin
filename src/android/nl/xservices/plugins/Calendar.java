...
public class Calendar extends CordovaPlugin {
  ...
  //private static final String ACTION_OPEN_EVENT = "openEvent";
  ...
  private static final int PERMISSION_REQCODE_FIND_ALL_EVENTS_IN_NAMED_CALENDAR = 203;
  ...
  public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
   ...
   /* if (ACTION_OPEN_EVENT.equals(action)) {
      if (hasLimitedSupport) {
        openCalendarLegacy(args);
      } else {
        openEvent(args);
      ...
      findEvents(args);
      return true;
	} else if (!hasLimitedSupport && ACTION_FIND_ALL_EVENTS_IN_NAMED_CALENDAR.equals(action)) {
  ...
  public void onRequestPermissionResult(int requestCode, String[] permissions, int[] grantResults) throws JSONException {
	...
	} else if (requestCode == PERMISSION_REQCODE_FIND_ALL_EVENTS_IN_NAMED_CALENDAR) {
      findAllEventsInNamedCalendar(requestArgs);
  ...
  @TargetApi(14)
  private void openEvent(JSONArray args) {
    try {
      final Long id = args.getJSONObject(0).optLong("id");

      cordova.getThreadPool().execute(new Runnable() {
        @Override
        public void run() {
          final Uri.Builder builder = CalendarContract.CONTENT_URI.buildUpon().appendPath("events");
  ...
  private void findAllEventsInNamedCalendar(JSONArray args) {
    ...
    // the app will ask permission and this method needs to be invoked again (done for backward compat).
    if (!calendarPermissionGranted(Manifest.permission.READ_CALENDAR)) {
      requestReadPermission(PERMISSION_REQCODE_FIND_ALL_EVENTS_IN_NAMED_CALENDAR);
      return;
    }
    ...
    try {
      ...
      cordova.getThreadPool().execute(new Runnable() {
        @Override
        public void run() {
          JSONArray jsonEvents = getCalendarAccessor().findAllEventsInNamedCalendar(getPossibleNullString("calendarName", jsonFilter));
